...
// Diğer API fonksiyonlarını da aynı şekilde güncelleyebilirsin.
...
export const getProductById = async (id: string): Promise<Product> => {
  try {/* json-server son sürümünde db dosyasında id string türünde olmalıdır. kontrol etmeyi unutmayalım */
    const res = await fetch(`http://localhost:4000/products/${id}`, {
      cache: "no-store",
    });
...
export const getValues = async () => {
  ...
  return {
    ...
    totalOrder:
      orderData.reduce(/* reduce ifadesi değerlerin yoplamını bulmak için kullanılır */
